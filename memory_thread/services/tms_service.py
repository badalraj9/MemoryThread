...
class TruthVectorService:
    ...
    def merge_vectors(v1: TruthVector, v2: TruthVector) -> TruthVector:
        ...
        Higher confidence vector has more weight.
        ...
        total_conf = v1.confidence + v2.confidence
        if total_conf == 0:
            w1, w2 = 0.5, 0.5
        else:
            w1 = v1.confidence / total_conf
            w2 = v2.confidence / total_conf
            
        return TruthVector(
            confidence=(w1 * v1.confidence + w2 * v2.confidence),
            authority=max(v1.authority, v2.authority),  # Take highest authority
            freshness=max(v1.freshness, v2.freshness),  # Take freshest
            corroboration=v1.corroboration + v2.corroboration + 1  # Increment for merge
        )
...
class StateDerivationService:
    ...
    def apply_event(current_state: EntityState, event: Event) -> EntityState:
        ...
        if event.object_id != current_state.entity_id:
            raise ValueError(f"Event object_id {event.object_id} doesn't match entity_id {current_state.entity_id}")
