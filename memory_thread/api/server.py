...
### Authentication
Use `X-API-Key` header with your client API key.
    """,
    version="1.0.0",
    contact={
...
async def remember(
    ...
    client: MemoryClient = Depends(get_client)
):
    """
...
async def recall(
    ...
    try:
        result = client.recall(
            query=request.query,
            top_k=request.top_k,
            min_truth_score=request.min_truth_score
        )
...
async def ingest_fact(
    ...
    try:
        ...
        return FactResponse(
            fact_id=fact_id,
            message="Fact ingested successfully"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def derive_belief(
    ...
    try:
        ...
        return BeliefResponse(
            belief_id=belief_id,
            message="Belief derived successfully"
        )
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def query_galaxy(
    request: GalaxyQueryRequest,
    client: MemoryClient = Depends(get_client)
):
    """
